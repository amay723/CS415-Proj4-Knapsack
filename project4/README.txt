...
Dynamic Programming space O(2W) mainEX1.cpp: We were able to construct the optimal value for the dynamic programming 
...
Greedy Algorithm mainEX2.cpp: We implimented a min heap in the greedy algorithm that stores the given 
weight/value/ratio pairs in order in nlogn time. The use of the heap saves us time from sorting the 
values after they have all been calculated. The final index values for the subsets were also implimented 
using a min heap, so that those values did not need to be sorted either before printing them out.
